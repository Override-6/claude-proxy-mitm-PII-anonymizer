...
Device (any OS)
  └─ Tailscale VPN (WireGuard)
       └─ tailscale-exit (mateo.corp, 100.64.0.1)
...
The VPS (`mateo.corp`) runs four Docker services managed by `docker-compose.yml`:

| Service | Role |
|---|---|
| `headscale` | WireGuard coordination server at `vpn.mateo.corp` |
...
scp mateo.corp:/srv/Projects/freelance/mateo/claude-pii-proxy/data/mitmproxy/mitmproxy-ca-cert.pem ~/Downloads/mitmproxy-ca.pem
```
...
# The installer does not always start the daemon — do it explicitly:
sudo systemctl enable --now tailscaled

# 2. Join the VPN (replace AUTHKEY with the key from step 2a)
sudo tailscale up \
  --login-server=https://vpn.mateo.corp \
  --authkey=AUTHKEY \
  --accept-dns=true \
...
3. Enter the control server URL: `https://vpn.mateo.corp`
...
3. Enter `https://vpn.mateo.corp` and confirm.
...
ssh mateo.corp "docker logs claude-mitm-proxy --tail 20 -f"
```
...
ssh mateo.corp "docker exec headscale headscale nodes list"

# Remove a node (use the ID from the list above)
ssh mateo.corp "docker exec headscale headscale nodes delete --identifier <ID>"

# Rename a node
ssh mateo.corp "docker exec headscale headscale nodes rename --identifier <ID> --new-name <name>"

# Expire a node immediately (forces re-auth)
ssh mateo.corp "docker exec headscale headscale nodes expire --identifier <ID>"

# Generate a new auth key (user ID 2 = maxime)
ssh mateo.corp "docker exec headscale headscale preauthkeys create -u 2 --reusable --expiration 365d"
```
