...
# Field names we never re-scan with MappingsEntityFinder: any jq path whose
# component list contains one of these would mutate a signed/hashed block
# (Claude's backend verifies thinking signatures against the exact bytes it
# emitted — see the "thinking signature" incident in the code history).
_MAPPINGS_FINDER_SKIP_FIELDS = frozenset({"thinking", "signature", "redacted_thinking"})

_mappings_finder = MappingsEntityFinder()


def _path_touches_signed_block(path: list) -> bool:
    """True if this jq path traverses a field whose bytes are signed/hashed
    by Claude's backend (thinking blocks). Mutating these invalidates the
    signature and causes the upstream request to fail."""
    return any(isinstance(p, str) and p in _MAPPINGS_FINDER_SKIP_FIELDS for p in path)


# Marker used to short-circuit the path walker when it enters a base64 image
# data field — see expand_paths.
_IMAGE_DATA_PATH_SUFFIX = ("source", "data")
...
async def _apply_paths(proxy: DLPProxy, data: dict, sensitive_fields: list[str] | bool, url: str = None) -> dict:
    ...
    # Drop paths that point at base64 image payloads: _anonymize_base64_images
    # has already redacted the pixels, so walking into `source.data` only
    # wastes NER compute on scrambled base64 and may produce phantom spans.
    paths = [
        p for p in paths
        if not (len(p) >= 2 and tuple(p[-2:]) == _IMAGE_DATA_PATH_SUFFIX)
    ]
    if not paths:
        return data
    ...
    # Re-run MappingsEntityFinder as a safety net: catches re-occurrences of
    # previously-seen PII that NER missed on this pass (NER is non-deterministic
    # across contexts — the same name can be caught once and missed the next
    # time depending on surrounding tokens).
    #
    # We SKIP any path that touches a signed/hashed field (thinking, signature,
    # redacted_thinking). Mutating those bytes invalidates Claude's backend
    # signature check and the upstream rejects the request.
    #
    # Safety question: does deanonymisation of thinking blocks on the RESPONSE
    # side (see rules.jsonc deanonymise.responses ".content[].thinking" and
    # the SSE deanon stream) cause a leak here?
    #   • SSE path: sse_fields does NOT include `.delta.thinking`, so streamed
    #     thinking reaches the client WITH tokens preserved. A client that
    #     replays the conversation sends back the anonymised form — nothing
    #     for the mappings finder to do anyway.
    #   • Non-SSE path: `.content[].thinking` IS deanonymised before reaching
    #     the client, so a client that stores the response and replays it
    #     will send real PII back inside `thinking`. We deliberately do NOT
    #     re-anonymise it here: doing so would mutate the bytes and the
    #     signature would no longer verify upstream. That PII was already
    #     generated by Claude from values Claude originally saw — it is not
    #     a new leak, just the same tokens round-tripping. The only true fix
    #     is to stop deanonymising `thinking` on responses at all; leave that
    #     decision to rules.jsonc.
    safe_indices = [i for i, p in enumerate(paths) if not _path_touches_signed_block(p)]
    if safe_indices:
        safe_values = [values[i] for i in safe_indices]
        for rel_idx, entities in enumerate(_mappings_finder.find_entities_batch(safe_values, proxy.mappings)):
            _add_non_overlapping(entities_list[safe_indices[rel_idx]], entities)
